...
float getInclination(float x1, float y1, float x2, float y2)
{
  float m = 0;
  if (y2 - y1 == 0)
    m = 0;
                                             ...
                                             Color color)
{
  ...
  // check bounds
  if (sx0 < 0 || sx0 >= target_w)
    return;
  if (sy0 < 0 || sy0 >= target_h)
    return;
  if (sx1 < 0 || sx1 >= target_w)
    return;
  if (sy1 < 0 || sy1 >= target_h)
    return;
  if (sx2 < 0 || sx2 >= target_w)
    return;
  if (sy2 < 0 || sy2 >= target_h)
    return;

  rasterize_line(sx0, sy0, sx1, sy1, color);
  rasterize_line(sx1, sy1, sx2, sy2, color);
  rasterize_line(sx2, sy2, sx0, sy0, color);

  float m0 = getInclination(sx0, sx1, sy0, sy1);
  float m1 = getInclination(sx1, sx2, sy1, sy2);
  float m2 = getInclination(sx2, sx0, sy2, sy0);
  // double a = atan2(m0, m1);
  printf("%d %d", sx1, sy1);
  Vector2D V1 = Vector2D(sx1 - sx0, sy1 - sy0);
  Vector2D V2 = Vector2D(sx2 - sx1, sy2 - sy1);
  Vector2D V0 = Vector2D(sx0 - sx2, sy0 - sy2);
  double a = cross(V1, V2);
  double b = cross(V2, V0);
  double c = cross(V0, V1);
  printf("%f %f %f\n", a, b, c);
  printf("asdfasdf %f %f %f %f asdfasdf\n", m0, m1, m2, a);
}
